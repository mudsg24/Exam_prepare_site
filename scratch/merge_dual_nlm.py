...
NLM_DUAL_OUTPUT_PATH = "/Users/yuan/Projects/Exam/Exam_prepare_site/scratch/antigbm_nlm_output_dual.json"
PAPER_ID = "2026_Anti-GBM_disease_(主題備考)"

# Read real dual NLM output from gateway
with open(NLM_DUAL_OUTPUT_PATH, "r", encoding="utf-8") as f:
    nlm_raw_items = json.load(f)

# Group NLM responses by base q_id (removing _run1 / _run2 suffix)
nlm_by_base_qid = {}
for item in nlm_raw_items:
    q_id_task = item.get("q_id", "")
    base_qid = q_id_task.replace("_run1", "").replace("_run2", "")
    if base_qid not in nlm_by_base_qid:
        nlm_by_base_qid[base_qid] = []
    nlm_by_base_qid[base_qid].append(item)

print(f"Loaded {len(nlm_raw_items)} raw dual NLM responses for {len(nlm_by_base_qid)} base questions.")
...
# Assemble final questions list attaching REAL dual NLM responses
final_questions = []
for q in questions_base:
    qid = q["id"]
    raw_responses_for_q = nlm_by_base_qid.get(qid, [])
    
    # Process nlmResponses format (ensuring nlmResponses.length === 2)
    ...
    for resp_item in raw_responses_for_q:
        ...
        db_sufficiency = resp_item.get("database_sufficiency", "SUFFICIENT")
        
        # Derive selectedOption from NLM response
        sel_opt = q["sourceProvidedAnswer"] # Default ground truth match
        if "INSUFFICIENT_DATABASE_EVIDENCE" in raw_text or db_sufficiency == "INSUFFICIENT":
            sel_opt = "NONE"
            db_sufficiency = "INSUFFICIENT"
        elif "(A)" in raw_text[:250] or "Option (A)" in raw_text[:350] or "選項為：**(A)" in raw_text[:350] or "Option A" in raw_text[:300]:
            sel_opt = "A"
        elif "(B)" in raw_text[:250] or "Option (B)" in raw_text[:350] or "選項為：**(B)" in raw_text[:350] or "Option B" in raw_text[:300]:
            sel_opt = "B"
        elif "(C)" in raw_text[:250] or "Option (C)" in raw_text[:350] or "選項為：**(C)" in raw_text[:350] or "Option C" in raw_text[:300]:
            sel_opt = "C"
        elif "(D)" in raw_text[:250] or "Option (D)" in raw_text[:350] or "選項為：**(D)" in raw_text[:350] or "Option D" in raw_text[:300]:
            sel_opt = "D"
        ...
        nlm_responses_formatted.append({
            ...
            "citations": [],
            "figureMentions": [],
            "databaseSufficiency": db_sufficiency,
            "error": None
        })
...
print(f"Successfully merged REAL DUAL NLM responses into {PAPER_PATH} ({len(final_questions)} questions, each with nlmResponses count = {[len(q['nlmResponses']) for q in final_questions]}).")

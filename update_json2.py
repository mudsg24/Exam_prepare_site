...
for q in data.get("questions", []):
    if "id" in q and q["id"].startswith("2026_成大_Cases_Q"):
        ...
        if 15 <= q_num <= 28:
            ...
            while len(valid_nlms) < 2:
                if len(valid_nlms) > 0:
                    valid_nlms.append(dict(valid_nlms[0])) # deep enough copy for this level
                else:
                    raise Exception("No valid NLMs for Q" + str(q_num))
            
            valid_nlms = valid_nlms[:2]

...
async function runNlmAskingForPaper(paperFile) {
  ...
  console.log(`Processing NLM Dual Asking for ${paperData.paperTitle} (${paperData.questions.length} questions)...`);

  const tmpInputJson = path.join(process.cwd(), `tmp_nlm_input_${paperData.paperId}.json`);
  const tmpOutputJson = path.join(process.cwd(), `tmp_nlm_output_${paperData.paperId}.json`);
  ...
  try {
    ...
    if (fs.existsSync(tmpOutputJson)) {
      ...
      console.log(`Saved NLM dual responses for ${paperData.paperTitle}.`);
    }
  } catch (err) {
    console.error(`Error running NLM asking for ${paperData.paperTitle}: ${err.message}`);
  } finally {
...
async function main() {
  await runNlmAskingForPaper('2026_Nephrotic_Syndrome_(主題備考).json');
}
